    ...
    if (this.data && this.data.length > 0) {
      this.flights = this.data.map((tour, index) => {
        ...
        return {
          ...
          airlineLogo: isEven
            ? 'https://placehold.co/50x50/003566/FFF?text=US'
            : 'https://placehold.co/50x50/ff4f5a/FFF?text=NV',
          ...
          outbound: {
            from:
              tour.route_name?.split('-')[0]?.substring(0, 3).toUpperCase() ||
              'DAC',
            to:
              tour.route_name?.split('-')[1]?.substring(0, 3).toUpperCase() ||
              'DST',
            depTime: tour.departure_at || '10:00 AM',
            depDate: tour.departure_time
              ? new Date(tour.departure_time).toDateString()
              : 'N/A',
            depAirport: 'Hazrat Shahjalal Int.',
            arrTime: tour.arrival_at || '11:30 AM',
            arrDate: tour.arrival_time
              ? new Date(tour.arrival_time).toDateString()
              : 'N/A',
            arrAirport: `${tour.location || 'Destination'} Airport`,
            duration:
              tour.departure_time && tour.arrival_time
                ? (() => {
                    const diff = Math.abs(
                      new Date(tour.arrival_time).getTime() -
                        new Date(tour.departure_time).getTime(),
                    );
